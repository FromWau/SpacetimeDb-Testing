...
class StatsExtrasTest {
    ...
    fun `procedureRequestTracker exists and starts empty`() = runBlocking {
        ...
        assertEquals(0, tracker.getSampleCount(), "No procedures called, sample count should be 0")
        assertNull(tracker.allTimeMin, "No procedures called, allTimeMin should be null")
        assertNull(tracker.allTimeMax, "No procedures called, allTimeMax should be null")
        assertEquals(0, tracker.getRequestsAwaitingResponse(), "No procedures in flight")
    ...
    fun `applyMessageTracker exists`() = runBlocking {
        ...
        assertTrue(tracker.getSampleCount() >= 0, "Sample count should be non-negative")
        assertTrue(tracker.getRequestsAwaitingResponse() >= 0, "Awaiting should be non-negative")
    ...
    fun `applyMessageTracker records after subscription`() = runBlocking {
        ...
        assertTrue(tracker.getSampleCount() >= 0, "Sample count should be non-negative after subscribe")
